...
export function createProductCard(product) {
  ...
  // accent + accent-fg flip between themes automatically:
  // light = mint fill / dark text · dark = forest fill / mint text
  const badge = soldOut
  ...
  return `
    <article class="group flex flex-col overflow-hidden rounded-xl bg-card shadow-sm ring-1 ring-line transition duration-300 hover:shadow-lg">
      ...
      <div class="relative overflow-hidden bg-accent-soft">
        ${badge}${isNewBadge}

        <a href="/pages/product.html?id=${product.id}" aria-label="${product.name}">
          <img src="${img1}" alt="${product.name}" loading="lazy"
               class="aspect-square w-full object-cover transition duration-500 ${imgStateClass} ${swapClass}" />
          ${
            img2 && !soldOut
              ? `
      ...
      <div class="flex flex-1 flex-col p-4">
        ...
        <a href="/pages/product.html?id=${product.id}"
           class="mt-1 min-h-11 font-medium leading-snug text-ink transition hover:text-accent-text-text line-clamp-2">
          ${product.name}
        </a>
